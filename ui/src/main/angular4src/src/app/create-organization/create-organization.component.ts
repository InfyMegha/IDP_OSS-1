  ...
  getAllOrganizations() {
	this.IdprestapiService.getExistingOrgNames().then(response => {
		...
		for (let i = 0; i < orgDetails.length; i++) {
			...
			if (org !== undefined && this.grantAccess.orgName !== undefined && org.toLowerCase() === this.grantAccess.orgName.toLowerCase()) {
			sameOrgFlag = true;
			}
		}
		if (sameOrgFlag) {
			if (document.getElementById("orgname").innerHTML !== null) {
			document.getElementById("orgname").innerHTML = "Organization already exists.";
			}
		} else {
			if (document.getElementById("orgname").innerHTML !== null) {
...
/* To create new Organiozation */
  go() {
	...
	if (this.flagEdit === true) {
		...
		this.IdprestapiService.editOrganizationDetails(this.grantAccess).then(response => {
		try {
			this.grantloading = false;
			if (response.status === -1 && response.statusText.toLowerCase() === "illegalstateexception") {
		...
		console.log(JSON.stringify(this.grantAccess));
		this.IdprestapiService.createOrganization(this.grantAccess).then(response => {
			try {
