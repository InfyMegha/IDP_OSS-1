...
@Component({
  ...
  styleUrls: ['./mail-success.component.css']
})
    ...
    this.IdprestapiService.getPipelineDetails(data)
      .then(response => {
        ...
        try {
          ...
          for (let i = 0; i < this.pipelineDetails.buildInfo.modules.length; i++) {
            console.log(this.pipelineDetails.buildInfo.modules[i]);
            let f = false;
            if (this.pipelineDetails.buildInfo.modules[i].codeAnalysis !== undefined) {

              for (let x = 0; x < this.pipelineDetails.buildInfo.modules[i].codeAnalysis.length; x++) {
                if (this.pipelineDetails.buildInfo.modules[i].codeAnalysis[x] !== null &&
                  this.pipelineDetails.buildInfo.modules[i].codeAnalysis[x] !== '' &&
                  this.pipelineDetails.buildInfo.modules[i].codeAnalysis[x] !== 'off') {
                  f = true;
                  break;
                }
              }
