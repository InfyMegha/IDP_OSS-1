...
export class IDPEncryption {
 ...
 arrayString=[];
  ...
  decryptAES(encTxt): any {
    const decrypted = CryptoJS.AES.decrypt(encTxt, this.key, { iv: this.iv, padding: CryptoJS.pad.NoPadding, mode: CryptoJS.mode.CBC});
  ...
  doubleEncryptPassword(data): any {
    const passwordDatalist = this.idpdataservice.passwordEncryptionList;
      ...
      for (let i in data['code']['scm'])
        for(let j in passwordDatalist['code.scm']) {
          let str = passwordDatalist['code.scm'][j];
          if (this.checkForKey(data['code']['scm'][i],str))
          {
            eval("data.code.scm[" + i + "]."+ str + " = " + "'" + this.encryptAES(eval("data.code.scm[" + i + "]."+ str)) + "'" );
          }
        }
    }
    if(data['code'].hasOwnProperty('buildScript')) {
      for (let i in data['code']['buildScript'])
        for (let j in passwordDatalist['code.buildScript']) {
          const str = passwordDatalist['code.buildScript'][j];
          if (this.checkForKey(data['code']['buildScript'][i],str)) {
            eval("data.code.buildScript[" + i + "]."+ str + " = " + "'" + this.encryptAES(eval("data.code.buildScript[" + i + "]."+ str)) + "'" );
          }
        }
    }

      for (let i in passwordDatalist['buildInfo']) {
        const str = passwordDatalist["buildInfo"][i];
          if (this.checkForKey(data["buildInfo"],str)) {
            eval("data.buildInfo." + str + " = " + "'" + this.encryptAES(eval("data.buildInfo." + str)) + "'" );
                }
      }
      if (data['buildInfo'].hasOwnProperty('modules')) {
        for (let i in data['buildInfo']['modules']) {
          for (let j in passwordDatalist['buildInfo.modules']) {
            let str=passwordDatalist['buildInfo.modules'][j];
            if (this.checkForKey(data['buildInfo']['modules'][i],str)) {
              eval("data.buildInfo.modules[" + i + "]." + str + " = " + "'" + this.encryptAES(eval("data.buildInfo.modules[" + i + "]." + str)) + "'" );
            }
          }
        }
      }
      if (data['deployInfo'].hasOwnProperty('deployEnv')) {
        for (let i in data['deployInfo']['deployEnv']) {
          if (data['deployInfo']['deployEnv'][i].hasOwnProperty('deploySteps')) {
            for (let j in data['deployInfo']['deployEnv'][i]['deploySteps']) {
              for (let k in passwordDatalist['deployInfo.deployEnv.deploySteps']) {
                let str = passwordDatalist['deployInfo.deployEnv.deploySteps'][k];
      ...
      {
        for(let i in data["testInfo"]["testEnv"])
        {
          if(data["testInfo"]["testEnv"][i].hasOwnProperty("testSteps"))
          {
            for(let j in data["testInfo"]["testEnv"][i]["testSteps"])
            {
              for(let k in passwordDatalist["testInfo.testEnv.testSteps"])
              {
                let str=passwordDatalist["testInfo.testEnv.testSteps"][k];
...
doubleDecryptPassword(data):any{
  let passwordDatalist=this.idpdataservice.passwordEncryptionList;
  //let data=dataJson.pipelineJson;
    ...
    for(let i in passwordDatalist["buildInfo"]){
      let str=passwordDatalist["buildInfo"][i];
        if(this.checkForKey(data["buildInfo"],str))
        {
