...
Initialize() {
    ...
    this.subscriptionService.getActiveServices().then(response => {
      ...
      data.forEach((value, index) => {
        ...
        if (value.serviceIdentity.serviceName === 'CI') {
          ...
          activeServiceInfo = 'Continuous Deployment to various servers and containers';
        } else if (value.serviceIdentity.serviceName === 'CT') {
          activeServiceInfo = 'Continuous Testing of tools for Functional , performance testing etc.';
        }
  ...
  applyForLicense() {
    ...
    if ( this.licenseKey.length > 0) {
      ...
      if ((isKeyValid = true)) {
        this.subscriptionService.addLicense(this.licenseKey).then(response => {
