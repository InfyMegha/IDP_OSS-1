...
@Component({
  ...
  styleUrls: ['./nodejs-cntrl.component.css']
})
export class NodejsCntrlComponent implements OnInit {
  ...
  checkCheckBox() {
    ...
    for (let i = 0; i < this.buildInfo.modules.length; i++) {
      ...
      if (this.buildInfo.modules[i].interval !== '' && this.buildInfo.modules[i].interval !== undefined) {
        incrementalScan = 'on';
        schdulePeriodicFullScan = 'on';

      }

      if (this.buildInfo.modules[i].relativePath !== null && this.buildInfo.modules[i].relativePath !== undefined &&
      ...
      if (this.buildInfo.modules[i].customScript !== null && this.buildInfo.modules[i].customScript !== undefined &&
      ...
      if (this.buildInfo.modules[i].pafFilePath !== '' && this.buildInfo.modules[i].pafFilePath !== undefined) {
        securityAnalysis = 'on';
      }

      if (this.buildInfo.modules[i].excludeFolders !== undefined && this.buildInfo.modules[i].excludeFolders !== '' ) {
        excludeDirToScan = 'on';
      }

      if (this.buildInfo.modules[i].npmProxy !== undefined && this.buildInfo.modules[i].npmProxy !== '') {
        depInstall =  'on';
      }

      if (this.buildInfo.modules[i].installGrunt !== undefined && this.buildInfo.modules[i].installGrunt !== '') {
        installGrunt = 'on';
      }
