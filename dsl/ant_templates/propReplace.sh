...
then
	echo "file found"
	. $1
	source_folder=`< $1 | grep SOURCEFOLDER | cut -d'=' -f2`
	sed -i -e 's/SRC_FOL/'${source_folder}'/g' cntl_file.txt
	source_repo=`< $1 | grep SOURCEREPO | cut -d'=' -f2`
	sed -i -e 's/SRC_REPO/'${source_repo}'/g' cntl_file.txt
	target_fol=`< $1 | grep TARGETFOLDER | cut -d'=' -f2`
	sed -i -e 's/TGT_FOL/'${target_fol}'/g' cntl_file.txt
	target_repo=`< $1 | grep TARGETREPO | cut -d'=' -f2`
	sed -i -e 's/TGT_REPO/'${target_repo}'/g' cntl_file.txt
	source_shrd=`< $1 | grep SOURCESHARED | cut -d'=' -f2`
	sed -i -e 's/SRC_SHRD/'${source_shrd}'/g' cntl_file.txt
	target_shrd=`< $1 | grep TARGETSHARED | cut -d'=' -f2`
	...
	imp_obj_shrd=`< $1 | grep IMPORTSHARED | cut -d'=' -f2`
	sed -i -e 's/IMP_SHRD_XML/'${imp_obj_shrd}'/g' INFA_CONFIG_IMPORT.sh
	port_imp=`< $1 | grep PORTT | cut -d'=' -f2`
	sed -i -e 's/PORT/'${port_imp}'/g' INFA_CONFIG_IMPORT.sh
	imp_obj_xml=`< $1 | grep IMPORTXML | cut -d'=' -f2`
