...
/**
 *
 * This class has the method to create the SCM Test job
 *
 */
...
class Scm_Test {
    ...
    /*
    * This method is used to call other child jobs to create a SCM Test job with all the configuration
    */
    ...
    void run(DslFactory factory, jsonData) {
        ...
        if (jsonData.code.scm.size() > 0) {
            if (scmFlag) {
                ...
                baseJobCreationForScmTest(factory, jsonData)
            }
        }
    }

    /*
     * this method is used to create the SCM test job in jenkins
     */

    private void baseJobCreationForScmTest(factory, jsonData) {

        String basepath = jsonData.basicInfo.applicationName + '_' + jsonData.basicInfo.pipelineName

        new BaseJobCreator(

                jobName: basepath + '/' + basepath + Constants.SCM_TEST_JOBNAMEPOSTFIX,
                jobDescription: ''
    ...
    private void invokeMultiScm(context, jsonData) {
        context.with {
            def testScmlist = getSCMList(jsonData.code.scm);
            //def list = scmTypeExists(testScmlist);

            def scmlistVal = testScmlist.values();
    ...
    */
    ...
    private HashMap<Integer, Scm> getSCMList(scmArr) {
        ...
        for (int i = 0; i < scmArr.size(); i++) {
            if (null != scmArr.getAt(i).testRepo && scmArr.getAt(i).testRepo.equalsIgnoreCase("on")) {
